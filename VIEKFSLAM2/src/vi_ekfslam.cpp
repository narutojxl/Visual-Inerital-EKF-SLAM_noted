...
namespace vi_ekfslam {
    ...
    VIEkfSlam::VIEkfSlam(ros::NodeHandle& pnh) : is_gravity_set(false), nh(pnh) {
    ...
    bool VIEkfSlam::loadParameters() {
        ...
        nh.param<double>("initial_covariance/velocity", velocity_cov, 100);
        ...
        cout << "R_imu_cam: " << endl
             << camera_imu_parameters.R_imu_cam << endl;
        cout << "t_imu_cam: " << endl
             << camera_imu_parameters.t_imu_cam << endl;
        cout << "camera intrinsics: " << endl
             << camera_imu_parameters.camera_intrinsics << endl;
        cout << "distortion_coeffs: " << endl
             << camera_imu_parameters.distortion_coeffs << endl;
    ...
    bool VIEkfSlam::initialize() {
        ...
        state_server.continuous_noise_cov.block<3, 3>(3, 3) =
            camera_imu_parameters.acc_noise * Matrix3d::Identity(); //typo, acc_noise
        ...
        //jxl imu initial acc bias
        state_server.imu_state.acc_bias(0) = 0.0;
        state_server.imu_state.acc_bias(1) = 0.0;
        state_server.imu_state.acc_bias(2) = 0.0;

        // state_server.state_cov.block<3,3>(0,0) = 100.0 * Matrix3d::Identity();
        // state_server.state_cov.block<2,2>(3,3) = 10. * M_PI/180. * 10. * M_PI/180.
        // * Matrix2d::Identity();
    ...
    void VIEkfSlam::initializeGravityAndBias() {
        ...
        for (const auto& imu_msg : imu_buffer) {
            Vector3d angular_vel = vectorMsg2Eigen(imu_msg.angular_velocity);
            Vector3d linear_acc = vectorMsg2Eigen(imu_msg.linear_acceleration);
        ...
        Quaterniond q_w_i0 =
            Quaterniond::FromTwoVectors(gravity_imu, -camera_imu_parameters.gravity); //NED IMU
    ...
    void VIEkfSlam::processIMUData(const double& time,
        ...
        //paper中的状态变量顺序是 x = [p, v, theta, ba, bg]
        //作者的状态变量是 x = [p, theta, v, ba, bg]
        Matrix3d R = state_server.imu_state.orientation.toRotationMatrix();
        Phi.block<3, 3>(0, 0) = Matrix3d::Identity();
        Phi.block<3, 3>(0, 6) = -0.5 * R * skewSymmetric(acc) * dtime2;
        ...
        // Phi * G 是268式中的Fi?
        // TODO 为何是这个形式?
        state_server.state_cov.block<15, 15>(0, 0) =
            Phi * state_server.state_cov.block<15, 15>(0, 0) * Phi.transpose() + Q;

        if (state_server.fature_states.size() > 0) {
            state_server.state_cov.topRightCorner(15, state_server.state_cov.cols() - 15) =
                Phi * state_server.state_cov.topRightCorner(15, state_server.state_cov.cols() - 15);
            state_server.state_cov.bottomLeftCorner(state_server.state_cov.rows() - 15, 15) =
                state_server.state_cov.bottomLeftCorner(state_server.state_cov.rows() - 15, 15) * Phi.transpose();
        }
        ...
        for (int i = 0; i < detect_marker_num; ++i) {
            ...
            if (map_id2obstimes.find(marker_ids[i]) == map_id2obstimes.end()) {
                ...
                if (map_id2obstimes[marker_ids[i]] >= 5) {
                    ...
                    undistortPoints(
                        marker_corners[i], camera_imu_parameters.camera_intrinsics,
    ...
    void VIEkfSlam::processMarkerData(
        ...
        for (auto marker_data : marker_meas) {
            ...
            auto find_iter = std::find_if(state_server.fature_states.begin(),
                                          state_server.fature_states.end(),
                                          [marker_data](const FeatureState& f) {
            ...
            // This is an old marker
                ...
                int slam_index =
                    std::distance(state_server.fature_states.begin(), find_iter);
                // cout << "slam_index : " << slam_index << endl;
                MatrixXd H_x_i =
                    MatrixXd::Zero(8, 15 + 6 * state_server.fature_states.size());
                ...
                //     cout << "marker " << state_server.fature_states[i].id << " cov: "
                //     << endl;
    ...
    void VIEkfSlam::stateAugmentation(const MarkerMeasData& marker_data) {
        ...
        FeatureState::initializePose(state_server.imu_state, marker_data,
                                     marker_feature.orientation,
                                     marker_feature.position);
    ...
    void VIEkfSlam::measurementJacobian(int index, const MarkerMeasData& meas,
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            // Corner in imu frame
            Vector3d p_i =
                R_w_i.transpose() * (R_w_m * corners_in_marker[i] + t_w_m - t_w_i);
            // Corner in camera frame
            Vector3d p_c = R_i_c.transpose() * (p_i - t_i_c);

            Matrix<double, 2, 3> jacobian_uv_xyz;
            // const double& fx =
            // camera_imu_parameters.camera_intrinsics.at<double>(0,0);
            // const double& fy =
            // camera_imu_parameters.camera_intrinsics.at<double>(1,0);
            // jacobian_uv_xyz << fx/p_c(2), 0, -p_c(0)*fx/(p_c(2)*p_c(2)),
            //                     0, fy/p_c(2), -p_c(1)*fy/(p_c(2)*p_c(2));
            jacobian_uv_xyz << 1.0 / p_c(2), 0, -p_c(0) / (p_c(2) * p_c(2)), 0,
                1.0 / p_c(2), -p_c(1) / (p_c(2) * p_c(2));
            MatrixXd jacobian_xyz_state = MatrixXd::Zero(3, total_state_size);
            jacobian_xyz_state.block<3, 3>(0, 0) =
                -R_i_c.transpose() * R_w_i.transpose();
            jacobian_xyz_state.block<3, 3>(0, 3) =
                R_i_c.transpose() * skewSymmetric(p_i);
            jacobian_xyz_state.block<3, 3>(0, 15 + 6 * index) =
                R_i_c.transpose() * R_w_i.transpose();
            jacobian_xyz_state.block<3, 3>(0, 15 + 6 * index + 3) =
                -R_i_c.transpose() * R_w_i.transpose() * R_w_m *
                skewSymmetric(corners_in_marker[i]);
    ...
    void VIEkfSlam::measurementUpdate(const MatrixXd& H, const VectorXd& r) {
        ...
        int total_state_size = 15 + 6 * state_server.fature_states.size();
        MatrixXd I_KH =
            MatrixXd::Identity(total_state_size, total_state_size) - K * H;
        // This formular is not stable here , why??
        // state_server.state_cov = I_KH * P * I_KH.transpose() + K * V *
        // K.transpose();
    ...
    void VIEkfSlam::updateState(const VectorXd& delta) {
        ...
        imu_state.orientation *= Quaterniond(
            AngleAxisd(delta.segment(3, 3).norm(), delta.segment(3, 3).normalized()));
        ...
        for (int i = 0; i < marker_cntr; ++i) {
            ...
            features[i].orientation *=
                Quaterniond(AngleAxisd(delta.segment(15 + 6 * i + 3, 3).norm(),
    ...
    void VIEkfSlam::publish(const ros::Time& time) {
        ...
        tf_pub.sendTransform(
            tf::StampedTransform(T_w_i_tf, time, world_frame_id, body_frame_id));
        ...
        tf::quaternionEigenToMsg(imu_state.orientation,
        ...
        for (int i = 0; i < features.size(); ++i) {
            ...
            drawRosMarker(
                features[i].id, time, features[i].position, features[i].orientation,
